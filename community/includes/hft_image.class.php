...
$hft_image_errors["UNABLE_TO_OUTPUT"]       = "Unable to output: ";
$hft_image_errors["FILE_DOESNOT_EXSIT"]     = "This file does not exist: ";
$hft_image_errors["FUNCTION_DOESNOT_EXIST"] = "This function does not exist: ";
$hft_image_errors["GD2_NOT_CREATED"]        = "GD2 is installed, function ImageCreateTruecolor() exists, but image is not created";
$hft_image_errors["IMG_NOT_CREATED"]        = "Image is not created ImageCreate(). {GD2 suppor is OFF}";
$hft_image_errors["GD2_UNAVALABLE"]         = "You specified to use GD2, but not all GD2 functions are present.";
$hft_image_errors["GD2_NOT_RESIZED"]        = "GD2 is installed, function ImageCopyResampled() exists, but image is not resized";
$hft_image_errors["IMG_NOT_RESIZED"]        = "Image was not resized. {GD2 suppor is OFF}";
$hft_image_errors["UNKNOWN_OUTPUT_FORMAT"]  = "This image format cannot bu output: ";
$hft_image_errors["NO_IMAGE_FOR_OUTPUT"]    = "Image you are trying to output does not exist.";
$hft_image_errors["IMG_NOT_SUPPORTED"]      = "Can not create image. Sorry, this image type is not supported yet.";
...
class hft_image {
    ...
    function hft_image( $file_original )
    {
        ...
        if ( file_exists($file_original) )
        {
            ...
            {
                $this->error($hft_image_errors["IMAGE_NOT_CREATED_FROM_FILE"] . " file = " . $file_original);
                return false;
            }
        }
        else
        {
            $this->error($hft_image_errors["FILE_DOESNOT_EXSIT"] . " file = " . $file_original);
            return false;
        }
    ...
    function imagecreatefromfile($img_file)
    {
        ...
        switch ( $img_sz[2] )
        {
            ...
            default:
                ...
                $this->error($hft_image_errors["IMG_NOT_SUPPORTED"] . " " . $img_file);
                break;
        } //case
    ...
    function _imagecheckandcreate( $function, $img_file )
    {
        ...
        {
            $img = false;
            $this->error($hft_image_errors["FUNCTION_DOESNOT_EXIST"] . " " . $function);
        }
    ...
    function resize( $desired_width, $desired_height, $mode = "-" )
    {
        ...
        // by desired_width, _height
        // >> mode = "-" then image is resized to fit into the region
        // specified by desired_width, _height
        ...
        switch($mode)
        {
            ...
            case '0':
                ...
                if ( $desired_width == "*" )
                    $desired_width = $this->image_original_width;
                if ( $desired_height == "*" )
                    $desired_height = $this->image_original_height;

                $new_width = $desired_width;
                $new_height = $desired_height;
                break;
            default:
                $this->error($hft_image_errors["UNKNOWN_RESIZE_MODE"] . " " . $mode);
                break;
        }
        ...
        if ( $this->use_gd2 )
        {
            ...
            {
                $this->image_resized = imagecreatetruecolor($new_width, $new_height) or $this->error($hft_image_errors["GD2_NOT_CREATED"]);
            }
            else
            {
                $this->error($hft_image_errors["GD2_UNAVALABLE"] . " ImageCreateTruecolor()");
            }
        }
        else
        {
            $this->image_resized = imagecreate($new_width, $new_height) or $this->error($hft_image_errors["IMG_NOT_CREATED"]);
        }
        ...
        if ( $this->use_gd2 )
        {
            ...
            {
                $res = imagecopyresampled($this->image_resized, $this->image_original,
                    0, 0,                                                     // dest coord
                    0, 0,                                                     // source coord
            ...
            {
                $this->error($hft_image_errors["GD2_UNAVALABLE"] . " ImageCopyResampled()");
            }
        }
        else
        {
            $res = imagecopyresized($this->image_resized, $this->image_original,
                0, 0,                                                     // dest coord
                0, 0,                                                     // source coord
                $new_width, $new_height,                                  // dest sizes
                $this->image_original_width, $this->image_original_height // src sizes
            ) or $this->error($hft_image_errors["IMG_NOT_RESIZED"]);
        }
        ...
        if ( $image )
        {
            switch ( $image_type )
            {
                ...
                case 'PNG':
                    ...
                    break;
                default:
                    $this->error($hft_image_errors["UNKNOWN_OUTPUT_FORMAT"] . " " . $image_type);
                    break;
            }
        }
        else
        {
            $this->error($hft_image_errors["NO_IMAGE_FOR_OUTPUT"]);
        }

        if ( !$res )
            $this->error($hft_image_errors["UNABLE_TO_OUTPUT"] . " " . $destination_file);
